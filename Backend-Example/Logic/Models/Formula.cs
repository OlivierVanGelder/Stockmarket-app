...
{
    public static double? CalculateFormula(double x, double mS)
    {
        try
        {
            var lD = mS % 10 + 1;
            var term1 =
                ...
                * 0.2;
            var term2 = Math.Abs(Math.Pow(Math.Cos(0.356 * x), 3));
            var term3 =
                ...
                + (0.25 + (mS % 2 == 0 ? -1 : 1 * mS * lD / 200) + lD / 10) * Math.Sin( 2 * Math.PI * (1 / ((10 + (mS / 100) * (120 - 10)) * 0.0006944444444)) * x );
